...
User-submitted translations are welcomed on this repository (see related section TBA).

## Using the tools to create badges
Files for badges are able to be generated using the Badge Tools menu on the YNOproject website.

To use it, access one of the games on the site, and append `?badge_tools=true` to the url (e.g. https://ynoproject.net/unconscious/?badge_tools=true).
